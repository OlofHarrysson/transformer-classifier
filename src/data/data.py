...
def setup_trainloader():
  ...
  dataset = MySTL10(dataset_dir, transforms)
  # dataset = MyCifar100(dataset_dir, transforms)
...
def setup_valloader():
  ...
  dataset_dir = get_project_root() / 'datasets'
  dataset = MySTL10(dataset_dir, transforms, split='test')
  # dataset = MyCifar100(dataset_dir, transforms, train=False)
  return DataLoader(dataset,
                    batch_size=cfg().batch_size,
                    num_workers=cfg().num_workers)
